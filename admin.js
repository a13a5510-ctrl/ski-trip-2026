// admin.js - SaaS 公版化邏輯 (Phase 3: 固定成本模組化)
...
class AdminPanel {
    ...
    listenToSettings() {
        onValue(ref(this.db, `${TRIP_ID}/settings`), (snapshot) => {
            ...
            if (data) {
                ...
                document.getElementById('s-nights').value = data.hotelNights || 4; // 🌟 載入晚數
            } else {
                document.getElementById('s-event-name').value = '新旅遊決策室'; document.getElementById('s-event-icon').value = '✈️';
                document.getElementById('s-token-name').value = '代幣'; document.getElementById('s-admin-pw').value = 'snow2026';
            }
        });
    }
    ...
    async submitNewCost(costData) {
        await set(push(ref(this.db, `${TRIP_ID}/costs`)), costData);
        document.getElementById('add-cost-form').reset();
    }
    
    async deleteCost(costId) {
        await remove(ref(this.db, `${TRIP_ID}/costs/${costId}`));
    }
    ...
    async forceCrown(hotelId, hotelName) { if ((await Swal.fire({ title: `加冕 ${hotelName}？`, text: '此舉將無視票數，直接讓該方案成為最終贏家！', icon: 'warning', showCancelButton: true, confirmButtonColor: '#eab308', confirmButtonText: '👑 強制加冕' })).isConfirmed) { await update(ref(this.db, `${TRIP_ID}/settings`), { winnerId: hotelId }); Swal.fire('加冕成功', '', 'success'); } }
    async clearCrown() { if ((await Swal.fire({ title: '撤銷加冕？', text: '系統將恢復以「最高票數」來決定贏家。', icon: 'question', showCancelButton: true, confirmButtonColor: '#eab308', confirmButtonText: '撤銷' })).isConfirmed) { await update(ref(this.db, `${TRIP_ID}/settings`), { winnerId: null }); Swal.fire('已撤銷', '', 'success'); } }
    async editHotel(hotelId) { 
        const hotel = this.currentHotelsData[hotelId]; if (!hotel) return;
        const { value: formValues } = await Swal.fire({
            title: `編輯: ${hotel.name}`, html: `<div class="space-y-4 text-left"><div><label class="block text-sm font-bold text-gray-700 mb-1">方案名稱</label><input id="edit-name" class="swal2-input !m-0 !w-full" value="${hotel.name}"></div><div><label class="block text-sm font-bold text-gray-700 mb-1">價格</label><input id="edit-price" type="number" class="swal2-input !m-0 !w-full" value="${hotel.price}"></div></div>`,
    ...
    async resetAllVotes() { if ((await Swal.fire({ title: '重置票數？', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: '歸零' })).isConfirmed) { const snapshot = await get(ref(this.db, `${TRIP_ID}/hotels`)); if (snapshot.exists()) { const updates = {}; Object.keys(snapshot.val()).forEach(key => updates[`${TRIP_ID}/hotels/${key}/totalVotes`] = 0); updates[`${TRIP_ID}/settings/winnerId`] = null; await update(ref(this.db), updates); Swal.fire('已歸零', '前端代幣將自動重置', 'success'); } } }
...
window.submitSettings = (e) => { 
    ...
    adminApp.updateSettings({ 
        eventName: document.getElementById('s-event-name').value, eventIcon: document.getElementById('s-event-icon').value,
        tokenName: document.getElementById('s-token-name').value, adminPassword: document.getElementById('s-admin-pw').value,
        defaultTokens: parseInt(document.getElementById('s-tokens').value) || 10, deadline: document.getElementById('s-deadline').value,
        hotelNights: parseInt(document.getElementById('s-nights').value) || 4 // 🌟 上傳晚數
    }); 
};

// 🌟 暴露出新增/刪除成本的 API
window.submitNewCost = (e) => {
    e.preventDefault();
    adminApp.submitNewCost({
